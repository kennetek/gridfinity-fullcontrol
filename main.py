...
design_name = f'1x{SPAN}x{HEIGHT}-gridfinity-fullcontrol'
nozzle_temp = 225
bed_temp = 85
print_speed = 1800
...
EH = 0.30  # extrusion height (and layer height)
initial_z = EH * 0.6  # initial nozzle position is set to 0.6x the extrusion height to get a bit of 'squish'
...
def layer(length, ro, ri, z, gap, odd=False, io=0, pre=None, post=None, cont=False):
...
def layer_bottom(length, ro, z, p, odd):
    ...
    for i in range(p)[::-1]:
        ...
        for o in range(4):
            if ro - (i + 0.5) * EW > 0:
                ...
                substeps.append(fc.Point(x=(2 * ((0b1100 >> o) & 1) - 1) * y, y=(2 * ((0b1001 >> o) & 1) - 1) * y, z=z))
        substeps.append(fc.Point(x=(i - 0.5) * EW, y=0.5 * ((2 * i + 1) * EW - length + 0.5), z=z))
    substeps.append(fc.Extruder(on=False))
    substeps.append(fc.Point(z=z + 0.1))
    substeps += fc.move_polar(infill(length - 0.5 - 2 * p * EW, z), centre=fc.Point(x=0, y=0, z=z), radius=0,
                              angle=odd * 0.5 * np.pi)
    substeps.append(fc.Extruder(on=False))
    substeps.append(fc.PrinterCommand(id="retract"))
    return substeps
...
def layer_shelf(length, ro, z, gap, io=0.0, layer=0):
    EWb = 0.5
    key = points_key(length, SPAN, ro, z)
    ri = ro - 1.5 * EW - io
    ...
    def layer_shelf_loop(point, vector, c1, c2, m):
        ...
        while True:
            ...
            l = list(filter(lambda p: c2.x - 0.01 <= p.x <= c1.x + 0.01 and c1.y - 0.01 <= p.y <= c2.y + 0.01, points))
            if not l:
                break
    ...
    def layer_shelf1():
        ...
        substeps = [fc.Point(x=0.75 * EW, y=yi - 0.5 * EWb, z=z), fc.Extruder(on=True), fc.PrinterCommand(id="unretract"),
                 fc.Printer(print_speed=print_speed_bridge), fc.Fan(speed_percent=100),
        ...
        substeps.append(fc.Printer(print_speed=print_speed * 0.6))
        substeps += layer_shelf_corner()

        substeps.append(fc.Printer(print_speed=print_speed))
    ...
    def layer_shelf2():
        ...
        substeps = [fc.Point(x=0.75 * EW, y=yi - 0.5 * EW, z=z), fc.Extruder(on=True), fc.PrinterCommand(id="unretract")]
...
def infill(length, z):
    ...
    points1 = [fc.Point(x=x, y=0.5 * d - np.abs(x - 0.5 * d), z=z) for x in
               np.linspace(0, d, int(np.ceil(d / (0.8 * EW))))]
...
def bottomFillA(length, ro, z):
    ...
    fill = fc.move(fill, fc.Vector(x=0, y=-LENGTH * 0.5, z=0), copy=False)
    return fill


# top part of filling the gap between bases
def bottomFillB(length, ro, z):
    fill = []
    kf = points_key(length, 1, ro, z)
    of = z - 1 - ro
    arc1 = fc.Arc(cx=kf[1].x + of, cy=kf[1].y + of * 2 - LENGTH * 0.5, cz=z, r=ro - 1.5 * EW - of, a=hp, a0=0)
    fill.append(arc1)
    arc2 = refy(arc1)
    arc_intersection(arc2, fc.Point(x=-kf[1].x - ro + 2.5 * EW, y=kf[1].y - LENGTH * 0.5, z=z),
...
for z in layers:
    ...
    h = 0
    if n < 3:
        first = layer_bottom(34.5 + 2 * r - 2 * first_layer_comp, r, z, 3, n % 2)
        ...
        if 2 <= idx_continuous_layer <= 4:
            ...
            if idx_continuous_layer % 2 == 0:
                ...
                temp_post = fc.move([refx(refy(p)) for p in temp], fc.Vector(x=0, y=0.5 * (SPAN - 2) * LENGTH, z=0))
                for i in range(SPAN - 1):
                    post += fc.move(temp_post, fc.Vector(x=0, y=-i * LENGTH, z=0))

        if r < 3.75:
            first = layer(34.5 + 2 * r, r, r - 1.5 * EW, z, z2s(z - h) - 0.5 * (41.5 - (34 + 2 * r)), side, z2l(z))
            first = fc.move(first, fc.Vector(x=0, y=LENGTH * (SPAN - 1) * 0.5, z=0), copy=False)
            steps += first
            if SPAN > 1:
                second = layer(34.5 + 2 * r, r, r - 1.5 * EW, z, 0, side, z2l(z))
            ...
            if idx_continuous_layer == 0:
                ...
                pre = []
                post = []
                temp_pre = bottomFillA(34.5 + 2 * r, r, z)
                ...
                for i in range(SPAN - 1):
                    pre += fc.move(temp_pre, fc.Vector(x=0, y=i * LENGTH, z=0))
                    post += fc.move(temp_post, fc.Vector(x=0, y=-i * LENGTH, z=0))
            steps += layer(34.5 + 2 * r, r, r - 1.5 * EW, z, z2s(z - h) - 0.5 * (41.5 - (34 + 2 * r)), side, z2l(z),
                           pre, post, idx_continuous_layer != 0)

        if idx_continuous_layer > 0:
            idx_continuous_layer += 1
    else:
        steps += layer_shelf(34.5 + 2 * r, r, z, 25, z2l(z), shelf)
        shelf_layers -= 1
        if shelf_layers <= 0:
            shelf = (n % 2) + 1
        # break
